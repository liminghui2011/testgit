...
public class EntityDaoSupport<T>{
	...
	public int queryInt(String hql ) {
	 ...
	 * @return
	 */
	protected PaginationBean<T> pageList(String dataHQL, Map<String, Object> params, int currentPage, int pageSize){
		
		SqlParameter sqlParamter =  new MapSqlParameter(params);
		String countHql = SELECT_COUNT_HEADER + dataHQL.substring(dataHQL.indexOf("from")) ;
		int totalCount = queryInt(countHql, sqlParamter);
		PaginationBean<T> pb = new PaginationBean<T>(currentPage, totalCount, pageSize);
		List<T> dataList = getRecordQuery(dataHQL, sqlParamter)
								.setFirstResult(pb.getStart())
